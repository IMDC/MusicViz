...
public class MidiNoteReceiver implements Receiver
{
	...
	public void send(MidiMessage m, long time)
	{
	    ...
	    {
	    	controller.getGUI().setCurrentValueForSlider(seconds);
	    	controller.getGUI().updateTimer(Utils.secondsToTime(seconds));
	    }
	    ...
	    if( status >= 224 && status <= 239 )
	    {
		    ...
		    if( (pitchBend - (double)initialPitchSettings[channel]) != 0.0)
		    {
			    ...
			    //controller.getGUI().getVisualizer().messageQueue.get(channel).add(pitchChange);
		    }
	    }
	    ...
	    else if ( status >= 144 && status <= 159 )
	    {
	    	...
	    	if( channel != 9 )
	    	{
	    		...
	    		if(seconds - lastInterval >= ThreadPreprocessor.threshold )
	    		{
		    		...
		    		controller.getGUI().getVisualizer().barQueue.add(position);
	...
	private  void tonalEventProcessing( int channel, int velocity, int note) 
   	{
		...
		if( channelsToUse[channel][tonalMessage.getPipe()] )
   		{
			controller.getGUI().getVisualizer().messageQueue.get(channel).add(tonalMessage);
   		}
   	}
