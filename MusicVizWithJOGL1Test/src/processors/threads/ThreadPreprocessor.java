...
public class ThreadPreprocessor extends SwingWorker<Void,Void>
{
	...
	{
		GUI gui = controller.getGUI();
		gui.getVisualizer().resetVisualizerInformation();
		Set<Integer> key = channelsUsed.keySet();
		gui.getVisualizer().setKeys( key );
		...
		for( int i = 0; i < changesByAmountString.length; i++ )
		{
			...
			for( int j = 0; j < numOfPipesPerTonalChannel; j++ )
			{
				...
				if( channel != 9 )
				{
					...
					gui.getVisualizer().setLastCoordinates(channel,j, coords);
					gui.getVisualizer().setLastDimensions(channel,j, dimension);
					gui.getVisualizer().setLastColourChannel(channel, colour.clone());

					//float t[] = {255.0f,0.0f,0.0f,0.0f};
					//colour = t;
					//for hong's looming
					gui.getVisualizer().setLastPitchInChannelAndPipe(channel,j, 0);
					gui.getVisualizer().setLastVolumeInChannelAndPipe(channel, j, 0);
					gui.getVisualizer().setLastLoomingPositionInChannelAndPipe(channel, j, 0);
		...
		gui.getVisualizer().setMappedChannels(partsMapped);
		gui.getVisualizer().setMappedBeats(beatPartsMapped);
		gui.getVisualizer().setChannelOrderWithXLimit(channelOrderWithXLimit);
		gui.getVisualizer().setNoteChangesInInterval(noteChangesInIntervalInteger);
		gui.getVisualizer().setInstrumentMinMax(instrumentMinMax);
	}
