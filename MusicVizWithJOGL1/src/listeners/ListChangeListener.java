...
public class ListChangeListener implements ListSelectionListener
{
	...
	public void valueChanged(ListSelectionEvent e) 
	{
		...
		Song currentSong = controller.getCurrentSongPlaying();
		Song tempSong;
		...
		{				
			...
			//the  song variable in the player to correspond this change.
      	    ...
      	    {
      	    	controller.getCurrentSongPlaying().setIndex(i);
      	    }
      	    tempSong.setIndex(i);
		}
		...
		{
			//disables the slider 
			controller.getSlider().setEnabled(false);
			//controller.setSliderCurrentValue(0); //Resets the slider to the beginning for anesthetics
			controller.getSlider().setValue(0);
			controller.getPlayPauseToggleButton().setText("Play ");	//tells the toggle button to be reset to play
			controller.getPlayPauseToggleButton().setSelected(false); //deselects the button because nothing is playing
		}
		else
		{
			controller.getSlider().setEnabled(true);
		}
	}
