...
public class MidiNoteReceiver implements Receiver
{
	...
	public void send(MidiMessage m, long time)
	{
	    ...
	    if( sequencer.isRunning() )
	    {
		    ...
		    controller.getSlider().setValue( seconds );
		    ...
		    /*//Math for current second time code.
		     ...
		     {
		    	 controller.updateTimer(min+":"+"0"+secondsOfAMin);
		     }
		     else
		     {
		    	 controller.updateTimer(min+":"+secondsOfAMin);
		     }*/
		    
		    controller.updateTimer(Utils.secondsToTime(seconds));
	    }
	...
	private  void tonalEventProcessing( int channel, int velocity, int note) 
   	{
		...
		if( channelsToUse[channel][tonalMessage.getPipe()] )
   		{
   			controller.getVisualizer().messageQueue.get(channel).add(tonalMessage);
   		}
   	}
