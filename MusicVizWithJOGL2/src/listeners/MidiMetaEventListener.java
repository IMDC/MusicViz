...
public class MidiMetaEventListener implements MetaEventListener
{
	...
	public void meta(MetaMessage event) 
	{
		...
		{	
			...
			controller.getPlaylist().setSelectedIndex(0);
			...
			DefaultListModel dlm = (DefaultListModel) controller.getPlaylist().getModel();

			//index of the current song in the list
			int index = controller.getCurrentSongPlaying().getIndex();
			...
			if( index + 1 <  dlm.getSize() )
			{
				...
				controller.getPlaylist().setSelectedIndex( index + 1 );
				...
				controller.startPreprocessing( ((Song) dlm.get(index + 1)),true ); 
			}
			else
			{
				 //If there is nothing else in the list
				controller.getSlider().setValue(0);
				controller.updateTimer("0:00");
				controller.changeSongPosition(0);
				...
				//it should loop
				ActionListener[] actionListeners = controller.getLoopCheckBox().getActionListeners();
				LoopingCheckBoxListener lcbl = (LoopingCheckBoxListener) actionListeners[0];
				
				controller.startPreprocessing( ( (Song) dlm.get(0) ),lcbl.getToLoop() ); 
			...
			controller.getPlaylist().repaint();
		}
		else if( type == 81 )
		{
